...
namespace Modulo
{
	namespace Core
	{
		namespace Communication
		{
			...
			MessagePassingCommunication(CommunicationType::TRANSFORMLISTENER, recipient, timeout, mutex),
			buffer_(clock)
			{
			...
			MessagePassingCommunication(CommunicationType::TRANSFORMLISTENER, std::make_shared<StateRepresentation::CartesianPose>(), timeout, mutex),
			buffer_(clock) 
			{
				this->tf_listener_ = std::make_unique<tf2_ros::TransformListener>(buffer_);
			}
