...
class EventHandler : public Modulo::Core::Cell
{
	...
	public:
		...
		Cell("event_handler", 1000ms, false)
		{
			this->sm = sent_sm;
		}
		...
		void step()
		{
		    ...
		    switch(e)
		    {
		    	case 3:
		          ...
		          break;
		        case 0:
		          this->sm->process_event(e0{});
		          break;
		        case 1:
		          this->sm->process_event(e1{});
		          break;
		        case 2:
		          this->sm->process_event(e2{});
		          break;
		    }
		}
};
...
int main(int argc, char * argv[])
{
	...
	static const auto start = [mh]
	{ 
		std::cout << "Starting" << std::endl;
		mh->activate();
	};

	static const auto act_home = [ma1, ma2, mh]
	{ 
		std::cout << "act_home: Activate move_home" << std::endl; 
		ma1->deactivate();
		ma2->deactivate();
		mh->activate();
	};

	static const auto act_action1 = [ma1, ma2, mh]
	{ 
		std::cout << "act_action1: Activate move_action1" << std::endl; 
		ma2->deactivate();
		mh->deactivate();
		ma1->activate();
	};

	static const auto act_action2 = [ma1, ma2, mh]
	{ 
		std::cout << "act_action2: Activate move_action2" << std::endl;
		mh->deactivate();
		ma1->deactivate();
		ma2->activate();
	};
	...
	struct state_machine 
	{
		auto operator()() const noexcept {
		  ...
		  return make_transition_table(
		  	...
		  	*"idle"_s + event<e_start> / start = "move_home"_s,
		    "move_home"_s + event<e1> / act_action1 = "move_action1"_s,
		    "move_home"_s + event<e2> / act_action2 = "move_action2"_s,
		    "move_action1"_s + event<e0> / act_home = "move_home"_s,
		    "move_action1"_s + event<e2> / act_action2 = "move_action2"_s,
		    "move_action2"_s + event<e0> / act_home = "move_home"_s,
		    "move_action2"_s + event<e1> / act_action1 = "move_action1"_s
		  );
		}
	};
